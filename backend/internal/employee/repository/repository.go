...
func (r Repository) List(ctx context.Context, companyPublicID string)([]model.Employee,error){ cid,err:=r.companyID(ctx,companyPublicID); if err!=nil{return nil,err}; rows,err:=r.DB.QueryContext(ctx,`SELECT public_id,employee_number,first_name,COALESCE(middle_name,''),last_name,COALESCE(gender,''),date_of_birth,COALESCE(national_id,''),COALESCE(passport_number,''),COALESCE(nationality,''),COALESCE(kra_pin,''),COALESCE(nssf_number,''),COALESCE(shif_number,''),COALESCE(nhif_number,''),employment_date,termination_date,COALESCE(job_title,''),COALESCE((SELECT d.code FROM departments d WHERE d.id=employees.department_id),''),COALESCE((SELECT d.name FROM departments d WHERE d.id=employees.department_id),''),COALESCE(employment_type,''),status,created_at FROM employees WHERE company_id=? ORDER BY first_name,last_name`,cid); if err!=nil{return nil,err}; defer rows.Close(); out:=[]model.Employee{}; for rows.Next(){var e model.Employee; if err:=rows.Scan(&e.PublicID,&e.EmployeeNumber,&e.FirstName,&e.MiddleName,&e.LastName,&e.Gender,&e.DateOfBirth,&e.NationalID,&e.PassportNumber,&e.Nationality,&e.KRAPIN,&e.NSSFNumber,&e.SHIFNumber,&e.NHIFNumber,&e.EmploymentDate,&e.TerminationDate,&e.JobTitle,&e.DepartmentCode,&e.Department,&e.EmploymentType,&e.Status,&e.CreatedAt);err!=nil{return nil,err}; s,err:=r.currentSalary(ctx,e.PublicID); if err!=nil{return nil,err}; e.CurrentSalary=s; out=append(out,e)}; return out,rows.Err() }

func (r Repository) Get(ctx context.Context, companyPublicID, employeePublicID string)(model.EmployeeDetail,error){ cid,err:=r.companyID(ctx,companyPublicID); if err!=nil{return model.EmployeeDetail{},err}; var e model.Employee; err=r.DB.QueryRowContext(ctx,`SELECT public_id,employee_number,first_name,COALESCE(middle_name,''),last_name,COALESCE(gender,''),date_of_birth,COALESCE(national_id,''),COALESCE(passport_number,''),COALESCE(nationality,''),COALESCE(kra_pin,''),COALESCE(nssf_number,''),COALESCE(shif_number,''),COALESCE(nhif_number,''),employment_date,termination_date,COALESCE(job_title,''),COALESCE((SELECT d.public_id FROM departments d WHERE d.id=employees.department_id),''),COALESCE((SELECT d.code FROM departments d WHERE d.id=employees.department_id),''),COALESCE((SELECT d.name FROM departments d WHERE d.id=employees.department_id),''),COALESCE(employment_type,''),status,created_at FROM employees WHERE company_id=? AND public_id=?`,cid,employeePublicID).Scan(&e.PublicID,&e.EmployeeNumber,&e.FirstName,&e.MiddleName,&e.LastName,&e.Gender,&e.DateOfBirth,&e.NationalID,&e.PassportNumber,&e.Nationality,&e.KRAPIN,&e.NSSFNumber,&e.SHIFNumber,&e.NHIFNumber,&e.EmploymentDate,&e.TerminationDate,&e.JobTitle,&e.DepartmentID,&e.DepartmentCode,&e.Department,&e.EmploymentType,&e.Status,&e.CreatedAt); if errors.Is(err,sql.ErrNoRows){return model.EmployeeDetail{},ErrNotFound}; if err!=nil{return model.EmployeeDetail{},err}; d:=model.EmployeeDetail{Employee:e}; var s model.Salary; err=r.DB.QueryRowContext(ctx,`SELECT public_id,CAST(basic_salary AS CHAR),pay_frequency,effective_from,effective_to FROM employee_salary_history WHERE employee_id=(SELECT id FROM employees WHERE public_id=?) AND effective_from<=CURDATE() AND (effective_to IS NULL OR effective_to>=CURDATE()) ORDER BY effective_from DESC LIMIT 1`,employeePublicID).Scan(&s.PublicID,&s.BasicSalary,&s.PayFrequency,&s.EffectiveFrom,&s.EffectiveTo); if err==nil{d.CurrentSalary=&s}else if !errors.Is(err,sql.ErrNoRows){return model.EmployeeDetail{},err}; return d,nil }
...
func (r Repository) currentSalary(ctx context.Context, employeePublicID string)(*model.Salary,error){
	var s model.Salary
	err:=r.DB.QueryRowContext(ctx,`SELECT public_id,CAST(basic_salary AS CHAR),pay_frequency,effective_from,effective_to FROM employee_salary_history WHERE employee_id=(SELECT id FROM employees WHERE public_id=?) AND effective_from<=CURDATE() AND (effective_to IS NULL OR effective_to>=CURDATE()) ORDER BY effective_from DESC LIMIT 1`,employeePublicID).Scan(&s.PublicID,&s.BasicSalary,&s.PayFrequency,&s.EffectiveFrom,&s.EffectiveTo)
