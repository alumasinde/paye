...
VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,pid,companyID,in.EmployeeNumber,in.FirstName,nilIf(in.MiddleName),in.LastName,nilIf(in.Gender),dateOrNil(in.DateOfBirth),nilIf(in.NationalID),nilIf(in.PassportNumber),nilIf(in.Nationality),nilIf(in.KRAPIN),nilIf(in.NSSFNumber),nilIf(in.SHIFNumber),nilIf(in.NHIFNumber),in.EmploymentDate,nilIf(in.JobTitle),nilIf(in.DepartmentID),nilIf(in.EmploymentType))
