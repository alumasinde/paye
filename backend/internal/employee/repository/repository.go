...
func (r Repository) Create(ctx context.Context, companyPublicID string, in model.CreateInput)(model.EmployeeDetail,error){
	...
	pid:=uuid.NewString()
	_,err=tx.ExecContext(ctx,`INSERT INTO employees(public_id,company_id,employee_number,first_name,middle_name,last_name,gender,date_of_birth,national_id,passport_number,nationality,kra_pin,nssf_number,shif_number,nhif_number,employment_date,job_title,department,employment_type)
VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,pid,companyID,in.EmployeeNumber,in.FirstName,nilIf(in.MiddleName),in.LastName,nilIf(in.Gender),dateOrNil(in.DateOfBirth),nilIf(in.NationalID),nilIf(in.PassportNumber),nilIf(in.Nationality),nilIf(in.KRAPIN),nilIf(in.NSSFNumber),nilIf(in.SHIFNumber),nilIf(in.NHIFNumber),in.EmploymentDate,nilIf(in.JobTitle),nilIf(in.Department),nilIf(in.EmploymentType))
