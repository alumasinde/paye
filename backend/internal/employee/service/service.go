...
func (s Service) SalaryHistory(ctx context.Context,c,u,e string)([]model.Salary,error){ if err:=s.require(ctx,c,u,"employees.read");err!=nil{return nil,err}; return s.Repo.SalaryHistory(ctx,c,e) }
func (s Service) AddSalary(ctx context.Context,c,u,e string,in model.SalaryInput)(model.Salary,error){ if err:=s.require(ctx,c,u,"employees.write");err!=nil{return model.Salary{},err}; in.BasicSalary=clean(in.BasicSalary); in.PayFrequency=strings.ToUpper(clean(in.PayFrequency)); in.EffectiveFrom=clean(in.EffectiveFrom); if err:=validateSalary(in);err!=nil{return model.Salary{},err}; return s.Repo.AddSalary(ctx,c,e,in) }

func clean(v string) string{return strings.TrimSpace(v)}
func normalizeCreate(in *model.CreateInput){ in.EmployeeNumber=clean(in.EmployeeNumber);in.FirstName=clean(in.FirstName);in.MiddleName=clean(in.MiddleName);in.LastName=clean(in.LastName);in.Gender=strings.ToUpper(clean(in.Gender));in.EmploymentDate=clean(in.EmploymentDate);in.BasicSalary=clean(in.BasicSalary);in.PayFrequency=strings.ToUpper(clean(in.PayFrequency));in.EffectiveFrom=clean(in.EffectiveFrom);in.KRAPIN=strings.ToUpper(clean(in.KRAPIN));in.DepartmentID=clean(in.DepartmentID) }
func normalizeUpdate(in *model.UpdateInput){ in.FirstName=clean(in.FirstName);in.MiddleName=clean(in.MiddleName);in.LastName=clean(in.LastName);in.Gender=strings.ToUpper(clean(in.Gender));in.Status=strings.ToUpper(clean(in.Status));in.EmploymentDate=clean(in.EmploymentDate);in.TerminationDate=clean(in.TerminationDate);in.KRAPIN=strings.ToUpper(clean(in.KRAPIN));in.DepartmentID=clean(in.DepartmentID) }
...
func validateUpdate(in model.UpdateInput)error{if in.FirstName==""||in.LastName==""||!validDate(in.EmploymentDate){return errors.New("first_name, last_name and employment_date are required")};if in.DateOfBirth!=""&&!validDate(in.DateOfBirth){return errors.New("date_of_birth must be YYYY-MM-DD")};if in.TerminationDate!=""&&!validDate(in.TerminationDate){return errors.New("termination_date must be YYYY-MM-DD")};if in.TerminationDate!="" && in.TerminationDate<in.EmploymentDate{return errors.New("termination_date cannot be before employment_date")};if in.Status==""{return errors.New("status is required")};return validateGender(in.Gender)}
func validateSalary(in model.SalaryInput)error{if !validDate(in.EffectiveFrom){return errors.New("effective_from must use YYYY-MM-DD")};n,e:=strconv.ParseFloat(in.BasicSalary,64);if e!=nil||n<0{return errors.New("basic_salary must be a non-negative number")};if in.PayFrequency==""{return errors.New("pay_frequency is required")};switch in.PayFrequency{case "MONTHLY","WEEKLY","BIWEEKLY":return nil};return errors.New("invalid pay_frequency")}
