...
func (s Service) UpdateCompany(ctx context.Context, companyID, userID string, in model.UpdateCompanyInput) (model.Company, error) {
	...
	if !ok {
		return model.Company{}, repo.ErrForbidden
	}
...
func (s Service) ListRoles(ctx context.Context, companyID, userID string) ([]model.Role, error) {
	if err := s.require(ctx, companyID, userID, "roles.read"); err != nil {
		return nil, err
	}
...
func (s Service) CreateRole(ctx context.Context, companyID, userID string, in model.CreateRoleInput) (model.Role, error) {
	if err := s.require(ctx, companyID, userID, "roles.write"); err != nil {
	...
	if in.Code == "" || in.Name == "" {
		return model.Role{}, errors.New("role code and name are required")
	}
	if !codePattern.MatchString(in.Code) {
		return model.Role{}, errors.New("role code may contain only letters, numbers, underscores and hyphens")
	}
...
func (s Service) ListMembers(ctx context.Context, companyID, userID string) ([]model.Member, error) {
	if err := s.require(ctx, companyID, userID, "members.read"); err != nil {
		return nil, err
	}
...
func (s Service) AddMember(ctx context.Context, companyID, userID, email, roleID string) (model.Member, error) {
	if err := s.require(ctx, companyID, userID, "members.write"); err != nil {
		return model.Member{}, err
	}
	email = strings.ToLower(strings.TrimSpace(email))
	roleID = strings.TrimSpace(roleID)
	if email == "" || roleID == "" {
		return model.Member{}, errors.New("email and role_id are required")
	}
...
func (s Service) UpdateMemberRole(ctx context.Context, companyID, userID, memberID, roleID string) (model.Member, error) {
	...
	if strings.TrimSpace(memberID) == "" || strings.TrimSpace(roleID) == "" {
		return model.Member{}, errors.New("member_id and role_id are required")
	}
...
func validateShared(country, currency, frequency string) error {
	if len(country) != 2 || !codePattern.MatchString(country) {
		return errors.New("country_code must be a two-character code")
	}
	if len(currency) != 3 || !codePattern.MatchString(currency) {
		return errors.New("currency_code must be a three-character code")
	}
