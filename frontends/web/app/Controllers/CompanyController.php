...
final class CompanyController
{
    public function index(): void
    {
        ...
        view('companies/index', [
            'title'=>'Companies','layout'=>'app','activeNav'=>'companies','user'=>auth_user(),
    ...
    public function showCreate(): void
    {
        ...
        view('companies/create', [
            'title'=>'Create company','layout'=>'app','activeNav'=>'companies','user'=>auth_user(),
    ...
    public function create(): void
    {
        ...
        if($input['legal_name']===''||$input['kra_pin']===''||!filter_var($input['email'],FILTER_VALIDATE_EMAIL)){
            Session::flash('error','Enter the legal company name, KRA PIN and a valid company email address.'); redirect('/companies/create');
        }
        if(!preg_match('/^[A-Z0-9]{8,15}$/',$input['kra_pin'])){
            Session::flash('error','Enter a valid KRA PIN using letters and numbers only.'); redirect('/companies/create');
        }
        $result=(new CompanyService())->create($input,auth_access_token());
        if(!$result['ok']){Session::flash('error',$result['message']);redirect('/companies/create');}
