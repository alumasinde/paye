...
<div class="table-wrap"><table><thead><tr><th>Employee</th><th>Employee no.</th><th>Department</th><th>Employment</th><th>Basic salary</th><th>Status</th><th></th></tr></thead><tbody>
<?php foreach($employees as $employee): ?><tr><td><strong><?= h(trim((string)($employee['first_name']??'').' '.(string)($employee['middle_name']??'').' '.(string)($employee['last_name']??''))) ?></strong><span class="muted"><?= h((string)($employee['job_title']??'—')) ?></span></td><td><?= h((string)($employee['employee_number']??'—')) ?></td><td><?= h((string)($employee['department']??'—')) ?></td><td><?= h((string)($employee['employment_type']??'—')) ?></td><td><?= isset($employee['current_salary']['basic_salary']) ? h(number_format((float)$employee['current_salary']['basic_salary'],2)) : '—' ?></td><td><span class="status status-approved"><?= h((string)($employee['status']??'ACTIVE')) ?></span></td><td class="row-action"><a class="text-link" href="/employees/show?company=<?= h($companyId) ?>&employee=<?= h((string)($employee['id']??'')) ?>">View</a></td></tr><?php endforeach; ?>
